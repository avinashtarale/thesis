...
main (int argc, char *argv[])
{
  ...
  uint32_t ue_responders_per_sector = 30; // Responder UEs per sectors
  ...
  uint32_t pscch_rbs = 22;         //* PSCCH pool size in RBs. Note, the PSCCH occupied bandwidth will be
  ...
  // Set pathloss model
  Config::SetDefault ("ns3::NistIndoorPropagationLossModel::CacheLoss", BooleanValue (true));
  lteHelper->SetPathlossModelType ("ns3::NistIndoorPropagationLossModel");
  ...
  for (gIt = createdgroups.begin() ; gIt != createdgroups.end() ; gIt++)
    {
      if(gIt->GetN() < nbreceivers){
          ...
          std::cerr << "Unable to satisfy group size requested for groupcast. Either the UEs are out of reach or"
        		       " there are not enough UEs!" << std::endl;
          return 1;
      }
      totalRxs += gIt->GetN ()-1;
